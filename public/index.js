    ...
    if((!nomeEditora.length > 0) || (!nomeEditora.length > 0)){
       alert("Um dos campos está em branco")
        ...
        let result = await fetch("/api/v1/publisher/", {
            method:'POST',
            body:JSON.stringify({"nomeFantasia": nomeEditora, "localDeOrigem": localDeOrigem}),
            headers:{
        ...
        if(response.has_error==true){
            ...
            response.data.forEach(element => {
                if(element.codigo == codigo){
                    document.getElementById('editarNomeFantasia').value=element.nomeFantasia
                    document.getElementById('editarLocalDeOrigem').value=element.localDeOrigem
...
btnEdit.addEventListener('click', async ()=>{
    try {
        ...
        let result = await fetch('/api/v1/publisher', {
            method:'PUT',
            body:JSON.stringify({nomeFantasia: document.getElementById('editarNomeFantasia').value, localDeOrigem:document.getElementById('editarLocalDeOrigem').value, codigo:document.getElementById('editarCodigo').value}),
            headers:{
                'Content-type':'application/json'
            }
        })
    ...
    if((!tituloLivro.length > 0) || (!dataLancamento.length > 0)){
       alert("Um dos campos está em branco")
        ...
        let result = await fetch("/api/v1/book", {
            method:'POST',
            body:JSON.stringify({"titulo": tituloLivro, "dataDeLancamento": dataLancamento}),
            headers:{
                'Content-type':'application/json'
            }
        })
        ...
        if(response.has_error == true){
            ...
            h3.classList.add("editoraItemTitle")
            let h4  = document.createElement('h4')
            h4.innerHTML = new Date(response.data.dataDeLancamento).toLocaleDateString()
            h4.classList.add("editoraItemSubTitle")
            divText.appendChild(h3)
            divText.appendChild(h4)
            divText.classList.add('editoraTexts')
        ...
        if(response.has_error==true){
            ...
            response.data.forEach(element => {
                if(element.id == id){
                    ...
                    document.getElementById('editarDataDeLancamento').value= new Date(element.dataDeLancamento).toLocaleDateString()
...
document.getElementById('saveEditLivro').addEventListener('click', async ()=>{
    try {
        let result = await fetch('/api/v1/book', {
            method:'PUT',
            body:JSON.stringify({titulo: document.getElementById('editarTituloDoLivro').value, dataDeLancamento:document.getElementById('editarDataDeLancamento').value, id:document.getElementById('idLivro').value}),
            headers:{
                'Content-type':'application/json'
            }
        })
